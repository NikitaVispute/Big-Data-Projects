...
object q1_twitter {
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 2) {
      println("Error: Please enter 2 args")
    }
    ...
    val IntervalSlide = new Duration(1 * 1000)
    ...
    val streamingContext = new StreamingContext(sparkContext, IntervalSlide)
    ...
    // Sentiment Analysis
    val tweets = tweetStream.map(x => (x.getText, GetSentiment.getSentiment(x.getText).toString))
    tweets.print()
